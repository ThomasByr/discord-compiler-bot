...
pub async fn compilers(ctx: &Context, msg: &Message, _args: Args) -> CommandResult {
  ...
  match compiler_manager.resolve_target(language) {
    ...
    RequestHandler::WandBox => {
      match compiler_manager.wbox.as_ref().unwrap().get_compilers(shortname_to_qualified(language))
      {
        Some(s) => {
          for lang in s {
  ...
  let avatar = {
    let data_read = ctx.data.read().await;
    let botinfo_lock =
      data_read.get::<ConfigCache>().expect("Expected BotInfo in global cache").clone();
