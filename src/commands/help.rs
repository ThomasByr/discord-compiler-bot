...
pub async fn help(ctx: &Context, msg: &Message, args: Args) -> CommandResult {
  ...
  if !args.is_empty() {
    ...
    let description = match cmd.as_str() {
      ...
      "cpp" | "c++" => {
        ...
        emb.field(
          "Example 1",
          format!("{}cpp {{ int a = 4; if (a > 3) {{ cout << \"true\"; }} }}", prefix),
          false,
        );
        ...
        emb.field(
          "Example 4",
          format!(
            "{}cpp int main() {{ cout << \"Main\"; f(); }} void f() {{ cout << \"f()\"; }}",
            prefix
          ),
          false,
        );
  ...
  msg.channel_id.send_message(&ctx.http, |m| {
        m.embed(|e| {
            ...
            e.field("format", "``` Formats code using a code formatter\n (i.e. clang-format or rustfmt) ```", false);
