...
pub async fn diff(ctx: &Context, msg: &ApplicationCommandInteraction) -> CommandResult {
  ...
  let message1_obj = ctx.http.get_message(msg.channel_id.0, message1_parse.unwrap()).await.ok();
  let message2_obj = ctx.http.get_message(msg.channel_id.0, message2_parse.unwrap()).await.ok();
