        ...
        data
          ...
          .components(|components| {
            components.create_action_row(|row| {
              row
                .create_button(|btn| {
                  btn.label("Invite me").style(ButtonStyle::Link).url(invite_link)
                })
                .create_button(|btn| {
                  btn.label("Vote for us").style(ButtonStyle::Link).url(dbl_link)
                })
                .create_button(|btn| btn.label("GitHub").style(ButtonStyle::Link).url(github_link))
                .create_button(|btn| btn.label("Stats").style(ButtonStyle::Link).url(stats_link))
            })
          })
      })
    })
