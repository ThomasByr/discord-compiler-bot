...
impl CommandRequest {
  pub fn new(command: &str, guild: Option<GuildId>) -> CommandRequest {
    ...
    CommandRequest { key: String::from(""), command: String::from(command), guild: guild_str }
  }
}
...
impl LanguageRequest {
  pub fn new(language: &str, fail: bool) -> LanguageRequest {
    LanguageRequest { key: String::from(""), language: String::from(language), fail }
  }
}
...
impl LegacyRequest {
  pub fn new(amount: Option<u64>) -> LegacyRequest {
    let request_type = if amount.is_some() { "servers" } else { "request" };
