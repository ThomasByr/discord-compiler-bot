...
impl CommandManager {
  pub fn new() -> Self {
    CommandManager { commands_registered: false, commands: CommandManager::build_commands() }
  }
  ...
  pub async fn register_commands_guild(&mut self, ctx: &Context, guild: &Guild) {
    match guild
      ...
      .await
    {
      Err(e) => error!("Unable to set application commands for guild '{}': {}", guild.id, e),
      Ok(commands) => info!("Registered {} commands in guild: {}", commands.len(), guild.id),
    }
  }
  ...
  pub fn build_commands() -> Vec<CreateApplicationCommand> {
    ...
    cmd
      ...
      .name(format!("Compile{}", if cfg!(debug_assertions) { " [BETA]" } else { "" }));
    cmds.push(cmd);

    cmd = CreateApplicationCommand::default();
    cmd
      .kind(CommandType::Message)
      .name(format!("Assembly{}", if cfg!(debug_assertions) { " [BETA]" } else { "" }));
    cmds.push(cmd);

    cmd = CreateApplicationCommand::default();
    cmd
      .kind(CommandType::Message)
      .name(format!("Format{}", if cfg!(debug_assertions) { " [BETA]" } else { "" }));
    cmds.push(cmd);

    cmd = CreateApplicationCommand::default();
    cmd
      .kind(CommandType::Message)
      .name(format!("Diff{}", if cfg!(debug_assertions) { " [BETA]" } else { "" }));
    cmds.push(cmd);
    ...
    cmd = CreateApplicationCommand::default();
    cmd.kind(CommandType::ChatInput).name("ping").description("Test my ping to Discord's endpoint");
    cmds.push(cmd);
