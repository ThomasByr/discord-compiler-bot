...
impl CompilationManager {
  pub async fn new() -> Result<Self, Box<dyn Error>> {
    ...
    Ok(CompilationManager { wbox: wbox.ok(), gbolt: gbolt.ok() })
  }
  ...
  pub async fn assembly(
    ...
    let options = RequestOptions {
      ...
      compiler_options: CompilerOptions { skip_asm: false, executor_request: false },
    ...
    match resolution_result {
      None => Err(CommandError::from(format!(
        "Target '{}' either does not produce assembly or is not currently supported on godbolt.org",
        target
      ))),
      Some(compiler) => {
        let response =
          Godbolt::send_request(&compiler, &parse_result.code, options.clone(), USER_AGENT).await?;
  ...
  pub async fn compiler_explorer(
    ...
    let options = RequestOptions {
      user_arguments: parse_result.options.join(" "),
      compiler_options: CompilerOptions { skip_asm: true, executor_request: true },
