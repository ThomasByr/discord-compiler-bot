...
impl CompilationManager {
  ...
  pub async fn assembly(
    ...
    match resolution_result {
      None => Err(CommandError::from(format!(
        "Target '{}' either does not produce assembly or is not currently supported on MS Azure",
        target
      ))),
      Some(compiler) => {
        let response =
  ...
  pub async fn compiler_explorer(
    ...
    let details =
      CompilationDetails { compiler: compiler.name.clone(), language: compiler.lang.clone() };
