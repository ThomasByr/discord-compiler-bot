  ...
  while let Some(c) = iter.next() {
    ...
    if *c == "<" {
      ...
      while let Some(stdin) = iter.next() {
        ...
        if *stdin == "<" {
          return Err(CommandError::from("`|`` operator should be last, unable to continue"));
        }
...
async fn get_url_code(url: &str, author: &User) -> Result<String, CommandError> {
  ...
  let response = match reqwest::get(url).await {
    Ok(b) => b,
    Err(_e) => {
      return Err(CommandError::from("GET request failed, perhaps your link is unreachable?"))
    }
  };
...
pub async fn get_message_attachment(
  ...
  if !attachments.is_empty() {
    ...
    return match reqwest::get(&attached.url).await {
      ...
      Err(e) => Err(CommandError::from(format!("Failure when downloading attachment: {}", e))),
    };
  }
