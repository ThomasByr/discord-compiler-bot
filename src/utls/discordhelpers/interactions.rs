  ...
  components
    .create_action_row(|row| {
      row.create_select_menu(|menu| {
        menu.custom_id("compiler_select").options(|opts| opts.set_options(compiler_options))
      })
    })
    .create_action_row(|row3| {
      row3
        .create_button(|button| button.label("Compile").style(ButtonStyle::Primary).custom_id(1))
...
pub async fn create_more_options_panel(
  ctx: &Context,
  interaction: Arc<MessageComponentInteraction>,
  parse_result: &mut ParserResult,
) -> Result<Option<Arc<ModalSubmitInteraction>>, CommandError> {
  interaction
  ...
  match target {
    ...
    RequestHandler::CompilerExplorer => {
      ...
      if list.is_none() {
        ...
        return Err(CommandError::from(format!("No suitable compilers found for: {}", language)));
      }
...
pub fn edit_to_confirmation_interaction<'a>(
  result: &CreateEmbed,
  resp: &'a mut EditInteractionResponse,
) -> &'a mut EditInteractionResponse {
  resp.set_embeds(Vec::from([build_publish_embed(), result.clone()])).components(|components| {
    components.set_action_rows(Vec::new()).create_action_row(|row| {
      row.create_button(|btn| btn.custom_id("publish").label("Publish").style(ButtonStyle::Primary))
    })
  })
}

pub fn create_language_interaction<'this, 'a>(
...
pub fn create_dismiss_response<'this, 'a>(
  resp: &'this mut CreateInteractionResponse<'a>,
) -> &'this mut CreateInteractionResponse<'a> {
  resp.kind(InteractionResponseType::UpdateMessage).interaction_response_data(|data| {
    data
      .set_embeds(Vec::new())
      .embed(|emb| {
        emb
  ...
  resp.kind(InteractionResponseType::ChannelMessageWithSource).interaction_response_data(|data| {
    data
      ...
      .components(|components| {
        components.create_action_row(|row| {
          row
            .create_button(|btn| btn.custom_id("cancel").label("Cancel").style(ButtonStyle::Danger))
        })
      })
  })
}
...
pub fn create_diff_response<'this, 'a>(
  ...
  resp.kind(InteractionResponseType::ChannelMessageWithSource).interaction_response_data(|data| {
    data.embed(|emb| {
      emb.color(COLOR_OKAY).title("Diff completed").description(format!("```diff\n{}\n```", output))
    })
  })
}
...
pub(crate) fn create_think_interaction(
  ...
  resp
    ...
    .embed(|emb| emb.color(COLOR_WARN).description("Processing request..."))
}

pub(crate) async fn handle_asm_or_compile_request<F, Fut>(
  ctx: &Context,
  ...
  Fut: Future<Output = Result<CreateEmbed, CommandError>>,
{
  ...
  if parse_result.target.is_empty() {
    ...
    let selection =
      match resp.await_component_interaction(ctx).timeout(Duration::from_secs(30)).await {
        Some(s) => s,
        None => return Ok(()),
      };
  ...
  if !sent_interaction {
    ...
    command
      .edit_original_interaction_response(&ctx.http, |response| {
        response.content("Select a compiler:").components(|c| {
      ...
      "2" => {
        command
          .edit_original_interaction_response(&ctx.http, |resp| {
            resp.components(|cmps| cmps.set_action_rows(Vec::new())).set_embeds(Vec::new()).embed(
              |emb| {
                emb.color(COLOR_WARN).description(
                  "Awaiting completion of modal interaction, \
        ...
        more_options_response =
          create_more_options_panel(ctx, interaction.clone(), &mut parse_result).await?;
  ...
  command.edit_original_interaction_response(&ctx.http, create_think_interaction).await.unwrap();
  ...
  if let Some(log) = comp_log_id {
    if let Ok(id) = log.parse::<u64>() {
      let guild = if command.guild_id.is_some() {
        command.guild_id.unwrap().0.to_string()
      } else {
        "<<unknown>>".to_owned()
      };
  ...
  if let Err(err) = command
    ...
    .await
  {
    return Err(CommandError::from(format!("Unable to update response: {}", err)));
  }
  ...
  if let Some(int) = int_resp.await_component_interaction(&ctx.shard).await {
    int.create_interaction_response(&ctx.http, create_dismiss_response).await?;
...
pub async fn send_error_msg(
  ...
  if edit {
    ...
    command
      .create_interaction_response(&ctx.http, |resp| {
        resp.kind(InteractionResponseType::ChannelMessageWithSource).interaction_response_data(
          |d| {
            d.content("")
              .set_embeds(Vec::new())
              .components(|cmps| cmps.set_action_rows(Vec::new()))
