    ...
    components
        .create_action_row(|row| {
            row.create_select_menu(|menu| {
                menu.custom_id("compiler_select")
                            ...
                            components
                                .create_action_row(|row2| {
                                    row2.create_input_text(|txt| {
                                        txt.custom_id("compiler_options")
...
pub fn edit_to_confirmation_interaction<'a>(
    ...
    resp.set_embeds(Vec::from([build_publish_embed(), result.clone()]))
        .components(|components| {
            components
                .set_action_rows(Vec::new())
                .create_action_row(|row| {
                    row.create_button(|btn| {
                        btn.custom_id("publish")
                            .label("Publish")
                            .style(ButtonStyle::Primary)
                    })
                })
            ...
            "2" => {
                command
                    .edit_original_interaction_response(&ctx.http, |resp| {
                        resp.components(|cmps| cmps.set_action_rows(Vec::new()))
                            .set_embeds(Vec::new())
                            .embed(|emb| {
