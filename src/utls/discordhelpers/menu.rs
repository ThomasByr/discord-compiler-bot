...
impl Menu {
  ...
  pub async fn run(&mut self) -> Result<(), CommandError> {
    let mut m = self
      ...
      .send_message(&self.ctx.http, |msg| {
        msg.set_embed(self.pages[self.page].clone()).set_components(self.components.clone())
      })
      .await?;

    let cib = m.await_component_interactions(&self.ctx.shard).timeout(Duration::from_secs(60));
  ...
  async fn update_msg(&self, msg: &mut Message) -> serenity::Result<()> {
    msg.edit(&self.ctx.http, |m| m.set_embed(self.pages[self.page].clone())).await
  }
  ...
  fn build_components() -> CreateComponents {
    ...
    c.create_action_row(|row| {
      row
        .create_button(|btn| btn.style(ButtonStyle::Primary).label("⬅").custom_id("left"))
        .create_button(|btn| btn.style(ButtonStyle::Primary).label("➡").custom_id("right"))
    });
    c
  }
}
