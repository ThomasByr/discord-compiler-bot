...
pub fn build_menu_items(
  ...
  while current_page < num_pages + 1 {
    ...
    for (i, item) in items[current_page * items_per_page..end].iter().enumerate() {
      ...
      //     item
      // ))
      writeln!(description, "**{}**) {}", current_page * items_per_page + i + 1, item).unwrap();
    }
    emb.color(COLOR_OKAY);
    emb.title(title);
    emb.description(description);
    emb.footer(|f| {
      f.text(&format!("Requested by {} | Page {}/{}", author, current_page + 1, num_pages + 1))
    });
...
pub async fn handle_edit(
  ...
  if let Ok(updated_message) = old.channel_id.message(&ctx.http, old.id.0).await {
    for reaction in &updated_message.reactions {
      if reaction.me {
        let _ =
          discordhelpers::delete_bot_reacts(ctx, &updated_message, reaction.reaction_type.clone())
            .await;
      }
    }
  }
  ...
  match success {
    true => {
      if let Some(success_id) = botinfo.get("SUCCESS_EMOJI_ID") {
        let success_name =
          botinfo.get("SUCCESS_EMOJI_NAME").expect("Unable to find success emoji name").clone();
        reaction =
          discordhelpers::build_reaction(success_id.parse::<u64>().unwrap(), &success_name);
      } else {
    ...
    false => {
      if let Some(fail_id) = botinfo.get("FAIL_EMOJI_ID") {
        let fail_name =
          botinfo.get("FAIL_EMOJI_NAME").expect("Unable to find fail emoji name").clone();
        reaction = discordhelpers::build_reaction(fail_id.parse::<u64>().unwrap(), &fail_name);
      } else {
        reaction = ReactionType::Unicode(String::from("❌"));
      }
    }
